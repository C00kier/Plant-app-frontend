...
export default function Recommendation({ token, userId, rooms }) {
  const { functionalityElement, setFunctionalityElement } = useContext(
    functionalityElementContext
  );
  const [userQuizAnswers, setUserQuizAnswers] = useState();
  const [recommendedPlants, setRecommendedPlants] = useState();
  const [shouldDisplayRecommeneded, setShouldDisplayRecommended] =
    useState(false);
    ...
    <>
      ...
      {userQuizAnswers !== undefined ? (
        <div id="recommended-plants-container">
          <span id="recommended-plants-title">
            Teraz możesz zobaczyć rekomendowane rośliny do Twojego wnętrza
          </span>
          <div id="communicate-container">
            <span id="plant-communicate">Rekomendowane rośliny</span>
            <span id="sun-communicate" className="criteria-communicate">
              Nasłonecznienie
            </span>
            <span id="mature-size-communicate" className="criteria-communicate">
              Wielkość<br></br> dorosłej rośliny
            </span>
            <span id="difficulty-communicate" className="criteria-communicate">
              Trudność
            </span>
            <span
              id="airpurifying-communicate"
              className="criteria-communicate"
            >
              Oczyszczanie <br></br>powietrza
            </span>
            <span id="toxic-communicate" className="criteria-communicate">
              Bezpieczny<br></br> dla zwierząt <br></br>i dzieci
            </span>
            <span
              id="add-to-my-plants-communicate"
              className="criteria-communicate"
            >
              Dodaj do<br></br> moich roślin
            </span>
          </div>
          <div id="plants-container">
            {shouldDisplayRecommeneded ? (
              recommendedPlants.map((plant, index) => (
                <RecommendedPlant
                  key={"recommendedPlant" + index}
                  plant={plant}
        ...
        <>
          <div className="cockpit-no-plants-container flex-column-center-center">
            ...
            <button
              ...
              type="button"
              onClick={() => setFunctionalityElement(COMPONENT_STATES.QUIZ)}
            >
              Wypełnij quiz!
            </button>
          </div>
