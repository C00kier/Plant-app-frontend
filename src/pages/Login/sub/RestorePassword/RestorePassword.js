    ...
    <>
      <div id="background-shade">
        <div id="restore-password-container">
          ...
          {displayEmailSend ? (
            <span id="reset-email-send">
              Wiadomość z resetem hasła została wysłana! Sprawdź email.
            </span>
          ) : (
            <>
              <span id="reset-email-info">
                Podaj swój email a wyślemy Ci link do zmiany hasła
              </span>

              <input
                onChange={(e) => {
                  handleIvalidEmailDisplay(e);
                }}
                type="email"
                id="restore-password-email"
              ></input>
              <div
                id="restore-password-bttn"
                onClick={async () => sendResetPasswordEmail()}
              >
                <span>Wyslij link!</span>
              </div>
              <span
                className="email-invalid-communicate"
                style={{ display: displayInvalidEmail }}
              >
                E-mail niepoprawny!
              </span>
              <span
                className="email-invalid-communicate"
                style={{ display: displayNotAllowed }}
              >
                Zmiana hasła niedostępna dla kont Google
              </span>
            </>
          )}
        </div>
